...
async def finalize_after_followups(uid: int, bot: Bot, state: FSMContext = None):
    """
    ...
    - отправляем результат пользователя.
    """
    ...
    try:
        ...
        # Переводим в состояние ожидания обратной связи
        if state:
            await state.set_state(InterviewFSM.awaiting_feedback)
        return  # Выходим из функции, ждем обратную связь
    except Exception as e:
        logging.exception(f"Ошибка запроса обратной связи: {e}")
        # Если не удалось запросить обратную связь, продолжаем без неё
        await continue_with_analysis(uid, bot, s)
...
async def on_poll_answer(pa: PollAnswer, bot: Bot):
    ...
    if all(pid in answered_ids for pid in s.followup_polls):
        # В poll_answer нет доступа к state, поэтому используем специальную функцию
        await request_feedback_without_state(uid, bot)

async def request_feedback_without_state(uid: int, bot: Bot):
    """Запрос обратной связи без возможности перевода в состояние"""
    s = USER_STATES.get(uid)
    if not s or not s.vacancy_key:
        return
    ...
    try:
        ...
        USER_STATES[uid] = s
        save_state(uid)
        # Не можем перевести в состояние, но пользователь может отправить сообщение
        return
    except Exception as e:
        logging.exception(f"Ошибка запроса обратной связи: {e}")
        # Если не удалось запросить обратную связь, продолжаем без неё
...
async def on_voice_answer(m: Message, state: FSMContext):
    ...
    # Проверяем, не был ли уже выполнен анализ
    if hasattr(s, 'analysis_completed') and s.analysis_completed:
        await m.answer("Интервью уже завершено. Используйте /start для начала нового интервью.")
        return
    ...
    # Проверяем, не завершились ли уже все вопросы
    vac = VACANCIES[s.vacancy_key]
    if s.q_index >= len(vac.questions):
        # Если все вопросы отвечены, но не запрошена обратная связь - запрашиваем её
        if not s.feedback_requested:
            await finalize_after_followups(m.from_user.id, m.bot, state)
        else:
            # Если обратная связь уже запрошена, но пользователь не в состоянии awaiting_feedback
            # значит это обратная связь - обрабатываем её
            await handle_feedback_voice(m, state)
        return
...
async def on_text_answer(m: Message, state: FSMContext):
    ...
    if s.q_index >= len(vac.questions):
        ...
        if not s.feedback_requested:
            ...
            # Если обратная связь уже запрошена, но пользователь не в состоянии awaiting_feedback
            # значит это обратная связь - обрабатываем её
            await handle_feedback_text(m, state)
        return
    
    await handle_answer_text(m, m.text or "")

async def handle_feedback_voice(m: Message, state: FSMContext):
    """Обработка голосовой обратной связи"""
    s = USER_STATES.get(m.from_user.id)
    if not s:
        await m.answer("Нажмите /start, чтобы начать.")
        return

    # Проверяем, не была ли уже получена обратная связь
    if hasattr(s, 'feedback_text') and s.feedback_text:
        await m.answer("Обратная связь уже получена. Отчёт готовится...")
        return
    ...
    try:
        ...
        os.makedirs("downloads", exist_ok=True)
        await m.bot.download_file(file.file_path, destination=local_path)
        feedback_text = await AudioService.process_audio_file(local_path)
        
        # Сохраняем обратную связь в состоянии
        s.feedback_text = feedback_text
        USER_STATES[m.from_user.id] = s
        save_state()
        
        # Продолжаем с анализом
        await continue_with_analysis(m.from_user.id, m.bot, s)
        
    except Exception as e:
        logging.exception(f"Ошибка обработки обратной связи: {e}")
        await m.answer("Не удалось обработать голосовое сообщение. Продолжаем без обратной связи.")
        # Продолжаем без обратной связи
        await continue_with_analysis(m.from_user.id, m.bot, s)

async def handle_feedback_text(m: Message, state: FSMContext):
    ...
    # Сохраняем текстовую обратную связь
    s.feedback_text = m.text.strip()
    USER_STATES[m.from_user.id] = s
    save_state()
...
@router.message(InterviewFSM.awaiting_feedback, F.content_type == "voice")
async def on_feedback_voice(m: Message, state: FSMContext):
    await handle_feedback_voice(m, state)

@router.message(InterviewFSM.awaiting_feedback, F.text)
async def on_feedback_text(m: Message, state: FSMContext):
    await handle_feedback_text(m, state)

# Обработчик для всех остальных типов сообщений в состоянии awaiting_feedback
@router.message(InterviewFSM.awaiting_feedback)
async def on_feedback_other(m: Message, state: FSMContext):
    await m.answer("Пожалуйста, отправьте голосовое или текстовое сообщение с вашими впечатлениями об интервью.")
...
async def handle_answer_text(m: Message, text: str):
    ...
    # Проверяем, что индекс не выходит за пределы списка вопросов
    if curr_idx >= len(vac.questions):
        # Если все вопросы уже отвечены, переходим к финализации
        await finalize_and_report(m, s)
        return
...
async def continue_with_analysis(uid: int, bot: Bot, s: InterviewState = None):
    """Продолжение анализа после сбора обратной связи"""
    if s is None:
        s = USER_STATES.get(uid)
        if not s:
            return
    ...
    free_answers = list(s.answers)
    quiz_answers = list(s.followup_answers or [])
    feedback_text = getattr(s, 'feedback_text', '')  # Получаем обратную связь из состояния

    # Теперь отправляем видео о начале анализа
    ...
    report = {
        ...
        "decision": decision,
        "rationale": rationale,
        "feedback": feedback_text,  # Добавляем обратную связь в отчет
    }
    ...
    # Помечаем анализ как завершённый
